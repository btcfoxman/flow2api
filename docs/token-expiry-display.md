...
`auth_mode=flow` 不使用 Labs AT，因此 `at_expires` 保持空值。管理页根据新字段显示已同步登录凭据的剩余有效期，旧 Labs 账号仍沿用 AT 显示。两者统一沿用原来的“天 / 小时 / 分钟”倒计时及颜色，悬停查看具体日期；不显示“Flow Cookie”或“Flow 新版”附注。
...
已到期条目仅在展示解析中保留，正常认证导入仍丢弃已到期 Cookie。纯会话 Cookie 显示“无固定期限”，不伪造日期；混合快照按有日期条目的最早到期时间显示倒计时，仅在悬停说明中注明另含会话项。非法或超出日期范围的元数据降级为到期未知。
...
2026-09-11 倒计时显示调整：12 项 JavaScript 回归通过，覆盖分钟 / 小时 / 天阈值、颜色、具体日期提示、已过期及未知状态；本地浏览器模拟接口验证 7 种展示状态和会话验证提示。此项仅调整前端，不改变同步或到期判定协议。
